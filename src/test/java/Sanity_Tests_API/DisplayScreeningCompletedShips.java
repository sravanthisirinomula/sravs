...
public class DisplayScreeningCompletedShips extends PTRAC_TestBase{
	...
	DisplayScreeningCompletedShips() throws EncryptedDocumentException, InvalidFormatException, IOException {
		/*Object data[][]=Utilities.getDatafromExcel(testDataFile, "ScreenCompletedShips");
		String apiKey=(String) data[0][1];
		String user=(String) data[0][0];
		*/
		api=baseURL+"/api/v1/transaction/?"+"status__in=H&order_by=-creation_date&api_key="+apiKey+"&username="+apiUser+"&limit="+limit;
		response=WebServices.Get(api);
		jsonPathEvaluator = response.jsonPath();
	}

	@Test
	void displayScreeningCompletedShips() throws IOException {
