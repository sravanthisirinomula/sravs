...
public class ScreeningDisplayShipsBasedOnStatusIMO extends PTRAC_TestBase{
	...
	ScreeningDisplayShipsBasedOnStatusIMO() throws EncryptedDocumentException, InvalidFormatException, IOException {
		...
		String imo_name=(String) data[0][3];	
		api=baseURL+"/api/v1/transaction/?"+"status__in="+status+"&ship__imo_number="+imo_name+"&order_by=-creation_date&api_key="+apiKey+"&username="+apiUser+"&limit="+limit;
		response=WebServices.Get(api);
		jsonPathEvaluator = response.jsonPath();
	}

	@Test
	void screeningDisplayShipsBasedOnStatusIMO() throws IOException {
	...
	void getShipIDs() throws IOException {
		...
		List<String> shipStatus=jsonPathEvaluator.get("objects.status");
		...
		for (int i=0; i<transactionID.size();i++) {
			...
			addInfoToReport("Ship Status: "+shipStatus.get(i));
		}
	}
}