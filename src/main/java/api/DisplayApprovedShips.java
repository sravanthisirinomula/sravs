...
public class DisplayApprovedShips extends PTRAC_API_TestBase{
	static String api;
	Response response;
	JsonPath jsonPathEvaluator;

	DisplayApprovedShips() throws EncryptedDocumentException, InvalidFormatException, IOException {
		Object data[][]=Utilities.getDatafromExcel(testDataFile, "ApprovedShips");
		String apiKey=(String) data[0][1];
		String user=(String) data[0][0];
		System.out.println(user);
		api=baseURL+"/api/v1/transaction/?"+"status__in=A&order_by=-creation_date&api_key="+apiKey+"&username="+user+"&limit="+limit;
