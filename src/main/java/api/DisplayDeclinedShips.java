...
public class DisplayDeclinedShips extends PTRAC_API_TestBase{
	static String api;
	Response response;
	JsonPath jsonPathEvaluator;

	DisplayDeclinedShips() throws EncryptedDocumentException, InvalidFormatException, IOException {
		Object data[][]=Utilities.getDatafromExcel(testDataFile, "DeclinedShips");
		String apiKey=(String) data[0][1];
		String user=(String) data[0][0];
		api=baseURL+"/api/v1/transaction/?"+"status__in=D&order_by=-creation_date&api_key="+apiKey+"&username="+user+"&limit="+limit;
		response=WebServices.Get(api);
		jsonPathEvaluator = response.jsonPath();
	}

	@Test
	void displayDeclinedShips() throws IOException {
	...
	void getShipIDs() throws IOException {
		...
		for (int i=0; i<transactionID.size();i++) {
			...
			addInfoToReport("Declined Ship IMO: "+IMO.get(i));
			addInfoToReport("Declined Ship Transaction ID: "+transactionID.get(i));
